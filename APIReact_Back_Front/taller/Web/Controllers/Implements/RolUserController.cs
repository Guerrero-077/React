...
namespace Web.Controllers.Implements
{
    ...
    public class RolUserController : BaseController<RolUserDto, IRolUserService>
    {
        public RolUserController(IRolUserService service, ILogger<RolUserController> logger)
            : base(service, logger) { }

        protected override Task<IEnumerable<RolUserDto>> GetAllAsync(GetAllType g) => _service.GetAllAsync(g);

        protected override Task<RolUserDto?> GetByIdAsync(int id) => _service.GetByIdAsync(id);
